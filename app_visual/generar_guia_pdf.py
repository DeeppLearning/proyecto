...
def build_pdf():
    ...
    story.append(Paragraph("• <b>Selector 'Dataset Activo':</b> Permite conmutar la vista entre el dataset de la <b>Industria TIC</b> (213.123 contratos reales) y el consolidado unificado.", bullet_style))
    ...
    story.append(Paragraph("• <b>Año Inicial / Final:</b> Filtra el periodo (2016 a 2025) recalculando al instante todas las gráficas y KPIs.", bullet_style))
    story.append(Paragraph("• <b>Departamento:</b> Aísla la contratación de una región en específico sobre los 32 departamentos del país.", bullet_style))
    ...
    story.append(Paragraph("• <b>Total Contratos (213.123):</b> Registros reales del sector TIC extraídos directamente desde las APIs oficiales de datos.gov.co (SECOP I y II) y procesados en Apache Parquet ZSTD.", bullet_style))
    story.append(Paragraph("• <b>Valor Contratado ($40.170,96 MM COP):</b> $40.17 Billones de pesos colombianos adjudicados en tecnología.", bullet_style))
    story.append(Paragraph("• <b>Promedio ($188.5 M COP):</b> Media aritmética del valor de un contrato tecnológico.", bullet_style))
    story.append(Paragraph("• <b>Mediana ($17.87 M COP - P50):</b> El 50% de los contratos cuesta $17.87 millones o menos.", bullet_style))
    story.append(Paragraph("• <b>Subsector y Dpto. Dominante:</b> Identifican que <i>Soporte y Consultoría TI</i> y <i>Cundinamarca</i> concentran la mayor asignación.", bullet_style))

    # 3. PESTAÑA 2: DISTRIBUCIÓN GEOGRÁFICA
    story.append(Paragraph("3. Pestaña: Distribución Geográfica", h1_style))
    story.append(Paragraph("Evidencia la distribución territorial del presupuesto tecnológico sobre los <b>32 departamentos de Colombia</b>:", body_style))
    story.append(Paragraph("• <b>Jurisdicción Líder Cundinamarca (62.4%):</b> Concentra 68.709 contratos por $25.055 MM COP.", bullet_style))
    story.append(Paragraph("• <b>Ranking y Matriz Territorial:</b> Lista interactiva ordenada de los 32 departamentos con sus contratos, montos y porcentajes (Antioquia 13.4%, Valle del Cauca 3.4%, Santander 2.1%).", bullet_style))
    ...
    story.append(Paragraph("• <b>Gráfico Doble Escala:</b> Eje izquierdo muestra el presupuesto total del Estado (~$140.000 a $210.000 MM COP anuales en ~3.1 millones de contratos); eje derecho muestra los $40.170,96 MM COP de TIC (213.123 contratos).", bullet_style))
    story.append(Paragraph("• <b>Porcentaje de Participación (Línea Verde):</b> La tecnología representa un promedio de entre el <b>2.4% y 3.7%</b> del gasto público nacional.", bullet_style))

    # 5. PESTAÑA 4: MODELOS ML & CONPES 4069
    story.append(Paragraph("5. Pestaña: Modelos de Machine Learning & CONPES 4069", h1_style))
    story.append(Paragraph("Aplica algoritmos de Inteligencia Artificial reales ejecutados sobre las 213.123 contrataciones:", body_style))
    story.append(Paragraph("• <b>Alineación CONPES 4069 (28.5% / $8.188,72 MM COP):</b> El 28.5% de los contratos de tecnología (60.685 contratos) incluye componentes auténticos de ciencia, investigación, desarrollo de software nuevo o analítica/IA.", bullet_style))
    story.append(Paragraph("• <b>Clustering K-Means (k=4):</b> Agrupa las compras en 4 perfiles: <i>Clúster 0</i> (Operación estándar, 207.176 contratos), <i>Clúster 1</i> (Contratos críticos con prórrogas masivas), <i>Clúster 2</i> (Outlier singular), y <i>Clúster 3</i> (Megacontratos tecnológicos estructurados).", bullet_style))
    story.append(Paragraph("• <b>Detección de Anomalías (Isolation Forest):</b> Identifica 2.129 contratos atípicos (1.0% de contaminación) por sobrecostos o plazos desmedidos.", bullet_style))
    ...
    story.append(Paragraph("• <b>Explorador de Contratos:</b> Motor de búsqueda en tiempo real con paginación server-side sobre los 213.123 registros en DuckDB.", bullet_style))
    story.append(Paragraph("• <b>Consola Analítica SQL:</b> Permite ejecutar sentencias SQL nativas sobre DuckDB en milisegundos.", bullet_style))
